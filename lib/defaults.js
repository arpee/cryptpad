...
Default.commonCSP = function (Env) {
    ...
    return [
        ...
        // Saito patch: allow specific embedders via permittedEmbedders config
        Env.enableEmbedding
            ? `frame-ancestors 'self' ${Env.protocol} vector:`
            : `frame-ancestors 'self' ${domain}${Env.permittedEmbedders && Env.permittedEmbedders !== Env.httpSafeOrigin ? ' ' + Env.permittedEmbedders : ''}`,
        `worker-src 'self'`,
        ""
    ];
};
